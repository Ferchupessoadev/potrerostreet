<x-layouts.app title="Registro">
    <div class="flex flex-col items-center justify-center min-h-[70vh] px-6">
        ...
        <div class="w-full max-w-sm">
            <form action="{{ route('register') }}" method="POST" class="p-8 bg-black border border-ps-gray/30 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                @csrf
                <h2 class="font-display text-4xl mb-8 text-white uppercase tracking-tighter">
                    Únete a <span class="text-ps-cyan">Potrero</span>
                </h2>

                <div class="space-y-4">
                    <div>
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Nombre completo" required
                               class="w-full p-3 bg-black border {{ $errors->has('name') ? 'border-red-500' : 'border-ps-gray' }} text-white focus:border-ps-cyan focus:outline-none transition-colors">
                        @error('name')
                    ...
                    <div>
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="Email" required
                               class="w-full p-3 bg-black border {{ $errors->has('email') ? 'border-red-500' : 'border-ps-gray' }} text-white focus:border-ps-cyan focus:outline-none transition-colors">
                        @error('email')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <input type="password" name="password" placeholder="Contraseña" required
                               class="w-full p-3 bg-black border {{ $errors->has('password') ? 'border-red-500' : 'border-ps-gray' }} text-white focus:border-ps-cyan focus:outline-none transition-colors">
                        @error('password')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="relative my-8">
                ...
                <div class="relative flex justify-center text-xs uppercase text-ps-gray"><span class="bg-black px-2">O regístrate con</span></div>
            </div>
